...
describe("get request...", () => {
  ...
  test("identifier field of blogs is named id", async () => {
    const response = await api.get("/api/blogs");
    const blogsWithIdField = response.body.filter(
...
describe("delete request...", () => {
  ...
  test("removes that blog from the db", async () => {
    ...
    assert.deepStrictEqual(
      responseAfter.body,
      response.body.filter((blog) => blog.id !== response.body[0].id),
    );
  });
});
describe("put request...", () => {
  ...
  test("correctly updates blog", async () => {
    ...
    assert.deepStrictEqual(
      responseAfter.body.filter((b) => b.id === updatedBlog1.id),
      [updatedBlog1],
    );
  });
});
